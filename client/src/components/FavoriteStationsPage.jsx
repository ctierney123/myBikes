...
const FavoriteStationsPage = () => {
  ...
  const openModal = async (station) => {
    ...
    try {
      ...
      const match = statusData.data.stations.find(
        (s) =>
          s.station_id === station.station_id || s.station_id === station.id
      );
        ...
        {favorites.map((station) => (
          <li key={station.id || station.station_id} className="station-card">
            ...
            <div className="button-group">
              <button className="view-btn" onClick={() => openModal(station)}>
                View Details
              </button>
              <button
      ...
      >
        {selectedStation && (
          <div className="modal-content">
            ...
            <p>
              <strong>Capacity:</strong> {selectedStation.capacity}
            </p>
            {stationStatus ? (
              <>
                <p>
                ...
                <p>
                  ...
                  {stationStatus.is_renting ? "Yes" : "No"}
                </p>
                <p>
                  <strong>Is Returning:</strong>{" "}
                  {stationStatus.is_returning ? "Yes" : "No"}
                </p>
                <p>
                  <strong>Last Reported:</strong>{" "}
                  {new Date(
                    stationStatus.last_reported * 1000
                  ).toLocaleString()}
                </p>
              </>
            ) : (
